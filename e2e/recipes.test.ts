...
test.describe('Recipe Management', () => {
	...
	test('image placeholders work correctly', async ({ page }) => {
		...
		const unexpectedErrors = errors.filter(
			(error) => !error.includes('Failed to load recipe') && !error.includes('404')
		);
...
test.describe('Admin Functionality', () => {
	test('admin routes require authentication', async ({ page }) => {
		const adminRoutes = ['/admin/recipes/new', '/admin/recipes/test-id/edit'];
...
test.describe('Error Handling', () => {
	...
	test('handles malformed URLs', async ({ page }) => {
		...
		const badUrls = ['/recipes/', '/admin/recipes/', '/admin/recipes//edit'];
