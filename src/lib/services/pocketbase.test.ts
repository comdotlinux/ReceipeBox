...
describe('PocketBaseService', () => {
	...
	describe('Error Handling', () => {
		it('should handle authentication errors', async () => {
			...
			await expect(pb.login({ email: 'test@example.com', password: 'wrong' })).rejects.toThrow(
				'Invalid credentials'
			);
		});
		...
		it('should handle field validation errors', async () => {
			...
			await expect(pb.login({ email: 'invalid', password: 'password' })).rejects.toThrow(
		...
		it('should handle unknown errors', async () => {
			...
			await expect(pb.login({ email: 'test@example.com', password: 'password' })).rejects.toThrow(
				'Network error'
			);
		});
	});
