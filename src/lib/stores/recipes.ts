...
class RecipesStore {
	...
	async updateRecipe(data: any): Promise<Recipe> {
		try {
			...
			// Update in the list
			recipes.update((existing) => existing.map((r) => (r.id === recipe.id ? recipe : r)));

			// Update current recipe if it's the same
			currentRecipe.update((current) => (current?.id === recipe.id ? recipe : current));
	...
	async deleteRecipe(id: string): Promise<void> {
		try {
			...
			currentRecipe.update((current) => (current?.id === id ? null : current));
		} catch (error) {
	...
	async extractFromUrl(url: string): Promise<any> {
		try {
			...
			extractionError.set(
				error instanceof Error ? error.message : 'Failed to extract recipe from URL'
			);
			throw error;
		} finally {
...
// Derived stores
export const featuredRecipes = derived(recipes, ($recipes) => $recipes.slice(0, 6));

export const totalRecipes = derived(
	searchResults,
	($searchResults) => $searchResults?.totalItems || 0
);

export const hasMoreRecipes = derived([searchResults, recipes], ([$searchResults, $recipes]) => {
